...
export function createEditorTheme(isDark: boolean): editor.IStandaloneThemeData {
  ...
  return {
    ...
    rules: [
      ...
      {
        token: 'comment',
        foreground: isDark ? darkTheme.comment.slice(1) : lightTheme.comment.slice(1),
      },

      // Keywords
      {
        token: 'keyword',
        foreground: isDark ? darkTheme.red.slice(1) : lightTheme.keyword.slice(1),
      },
      {
        token: 'keyword.control',
        foreground: isDark ? darkTheme.magenta.slice(1) : lightTheme.keyword.slice(1),
      },

      // Strings
      {
        token: 'string',
        foreground: isDark ? darkTheme.greener.slice(1) : lightTheme.string.slice(1),
      },

      // Numbers and constants
      {
        token: 'number',
        foreground: isDark ? darkTheme.orange.slice(1) : lightTheme.constant.slice(1),
      },
      {
        token: 'constant',
        foreground: isDark ? darkTheme.orange.slice(1) : lightTheme.constant.slice(1),
      ...
      {
        token: 'function',
        foreground: isDark ? darkTheme.lightBlue.slice(1) : lightTheme.entity.slice(1),
      },
      {
        token: 'class',
        foreground: isDark ? darkTheme.lightBlue.slice(1) : lightTheme.entity.slice(1),
      ...
      {
        token: 'variable',
        foreground: isDark ? darkTheme.white.slice(1) : lightTheme.variable.slice(1),
      },
      {
