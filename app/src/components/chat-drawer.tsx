            ...
            <div key={group}>
              ...
              {groupChats.reverse().map((chat: Chat) => (
                <button
                  key={chat.uuid}
                  value={chat.uuid}
                  className={`group truncate text-xs h-8 ps-2 z-20 justify-start w-[100%] rounded-lg text-muted-foreground transition-all whitespace-nowrap text-nowrap duration:100 hover:bg-accent items-center ${props.uuid === chat.uuid && 'text-foreground bg-accent/50'}`}
                  onClick={() => {
