      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        ...
        <Select value={sizeFilter} onValueChange={setSizeFilter}>
          ...
          <SelectContent>
            ...
            {Array.from(new Set(modelsData.flatMap(m => m.sizes))).sort().map(size => (
        ...
        <Select value={capabilityFilter} onValueChange={setCapabilityFilter}>
          ...
          <SelectContent>
            ...
            {Array.from(new Set(modelsData.flatMap(m => m.capabilities))).sort().map(capability => (
          ...
          >
            ...
            <div className='flex w-full justify-between'>
              <p className="text-xs text-muted-foreground mb-2 flex gap-1 items-center">
                <Clock className="size-3" />{getRelativeTimeString(model.published)}
              </p>
              <p className="text-xs text-muted-foreground mb-2 flex gap-1 items-center">
                <ArrowDownToLine className="size-3" />{model.pulls} downloads
              </p>
            </div>
                  ...
                  <DialogDescription>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      ...
                      {selectedModel.sizes.map(size => (
                        <Badge key={size} variant="outline" className="border-primary bg-primary/10 text-primary">
                          {size}
                        </Badge>
                      ))}
                    </div>
